...
// Types d'exercices d'ergothérapie
const exerciseTypes = [
  { value: 'all', label: 'Tous les exercices' },
  { value: 'daily-activities', label: 'Activités quotidiennes' },
  { value: 'fine-motor', label: 'Motricité fine' },
  { value: 'cognitive', label: 'Fonctions cognitives' },
  { value: 'adaptation', label: 'Adaptation environnement' },
...
export default function ErgoPage() {
  ...
  useEffect(() => {
    async function loadExercises() {
      try {
        const response = await fetch('/api/exercises/ergo')
        const data = await response.json()
        setExercises(data.exercises || [])
      } catch (error) {
        logger.error(error, 'Error loading ergo exercises')
      } finally {
        setLoading(false)
      }
    }
  ...
  if (loading) {
    return (
      <div className="container py-8">
        <div className="mb-8">
          <h1 className="mb-2 text-3xl font-bold">Exercices d&apos;Ergothérapie</h1>
          <p className="text-muted-foreground">
            Développez votre autonomie et facilitez vos activités du quotidien
          </p>
        </div>
