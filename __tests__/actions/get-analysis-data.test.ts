...
describe("getAnalysisData", () => {
  ...
  it("returns analysis data with default filters", async () => {
    ...
    const easyDiff = result.difficultyDistribution.find(
      (d) => d.difficulty === "easy"
    );
  ...
  it("filters data by exercise type", async () => {
    ...
    const whereClause = vi.mocked(prisma.exerciseAttempt.findMany).mock
      .calls[0][0]?.where;
  ...
  it("handles time range filters correctly", async () => {
    ...
    const call7d = vi.mocked(prisma.exerciseAttempt.findMany).mock.calls[0][0]
      ?.where;
    ...
    const call90d = vi.mocked(prisma.exerciseAttempt.findMany).mock.calls[0][0]
      ?.where;
    ...
    const callAll = vi.mocked(prisma.exerciseAttempt.findMany).mock.calls[0][0]
      ?.where;
